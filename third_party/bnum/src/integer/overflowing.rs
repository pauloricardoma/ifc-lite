...
impl<const S: bool, const N: usize, const B: usize, const OM: u8> Integer<S, N, B, OM> {
    ...
    pub const fn overflowing_mul(self, rhs: Self) -> (Self, bool) {
        ...
        //
        // ifc-lite fork (third_party/bnum/README.md): choose the schoolbook digit
        // width by target. wasm32 has no 128-bit multiply, so a u128-digit partial
        // product lowers to a `__multi3` software libcall — the dominant cost of the
        // exact CSG kernel on wasm. u32 digits make each partial product a single
        // `i64.mul` (no libcall), measured -9% total single-thread geometry time on
        // a CSG-dense model. The result is BYTE-IDENTICAL: `to_digits::<D>` is a
        // byte reinterpretation, so the low-N-byte product and the overflow flag
        // (a property of the true product) are independent of the digit width D.
        // Native keeps u128 digits, so native codegen is unchanged.
