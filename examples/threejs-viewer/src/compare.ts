...
import { buildDataFingerprint } from './data-fingerprint.js';
...
async function loadModel(file: File, modelId: ModelId): Promise<LoadedModel> {
  ...
  for (const [localExpressId, meshSegments] of meshesByExpressId) {
    ...
    entitiesByKey.set(comparisonKey, {
      ...
      dataFingerprint: fingerprintEntityData(store, localExpressId, ifcType, name),
      meshSegments,
      store,
    });
  }
...
/**
 * Adapter: read one entity out of the data store and hand it to the canonical
 * fingerprint in `./data-fingerprint.js` (a copy of `@ifc-lite/diff`'s — see
 * the header of that file). All the store-specific work is here; all the
 * hashing rules — sorting, normalization, what counts as the identity of an
 * assigned type — live there, so this demo and the package agree on the hash.
 */
function fingerprintEntityData(store: IfcDataStore, expressId: number, ifcType: string, name: string): string {
  const namedAttributes = extractAllEntityAttributes(store, expressId);
  // PredefinedType is an IFC enumeration, so it is a string in practice; the
  // attribute extractor's return type is wider, hence the String() normalize.
  const predefinedTypeValue = namedAttributes.find((attribute) => attribute.name === 'PredefinedType')?.value;
  const predefinedType = predefinedTypeValue == null ? '' : String(predefinedTypeValue);

  const typeAssignments = store.relationships
    .getRelated(expressId, RelationshipType.DefinesByType, 'inverse')
    .map((typeId) => ({
      // Semantic identity only. The type's own GlobalId is deliberately not
      // collected: it is regenerated by a from-scratch re-export just like a
      // product's, so hashing it would move the fingerprint of every typed
      // element on exactly the re-export this comparison is meant to survive.
  ...
  return buildDataFingerprint({
    ...
    predefinedType,
    propertySets: extractPropertiesOnDemand(store, expressId)
      .map((set) => ({ name: set.name, properties: [...set.properties] })),
    quantitySets: extractQuantitiesOnDemand(store, expressId)
      .map((set) => ({ name: set.name, quantities: [...set.quantities] })),
    typeAssignments,
  });
}

/**
 * Sampled 32-bit drift hash over the meshes of one entity.
 *
 * Deliberately narrower and cheaper than the data fingerprint in
 * `./data-fingerprint.js`: this value is only ever compared between the base
 * and the revised copy of an entity that already matched on GlobalId, so a
 * collision costs a missed "Geometry" flag on one element. The data
 * fingerprint is 64-bit because content matching treats hash equality as
 * *identity* across the whole model, where a collision pairs two unrelated
 * entities. Do not reuse this one for identity.
 */
